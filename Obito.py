...
def banner():
	clear()
	print("""
\033[1;37m____  ____ _____ _______ ____  
  / __ \|  _ \_   _|__   __/ __ \ 
 | |  | | |_) || |    | | | |  | |
 | |  | |  _ < | |    | | | |  | |
 | |__| | |_) || |_   | | | |__| |
...
def approval():
  ...
  print ("""\033[1;37m--------------------------------------------------""")
  try:
    httpCaht = requests.get("https://raw.githubusercontent.com/Rupesh-Don-Here/approval/main/Approval.txt").text
    if id in httpCaht:
      animation(">> Your Key Has Been Approved !!!")
      animation(">> Special Thanks To Harry For File create Menu<3 !!!")
      msg = str(os.geteuid())
      time.sleep(1)
      pass
    else: 
      ...
      os.system('xdg-open https://www.facebook.com/bhisnu.bhattarai?mibextid=ZbWKwL ')
      time.sleep(1)
      exit()
  except: 
	...
	if kz in ['1']:
		...
		if len(vin)==0:
			...
			try:geh = lol[geeh]
				...
				print('• RIGHT OPTION CHHOSE GARNA MUJI !!')
				exit()
...
def dump_massal():
	...
	try:
		linex()
		print(f' [\u001b[36m•\033[1;37m] Total Id Collected \33[1;32m'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		linex()
		animation(' [×] Network Error ')
